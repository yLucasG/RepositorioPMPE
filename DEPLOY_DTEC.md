...
- **Seções 2–3 (abaixo):** com Docker — mais simples de instalar do zero, não precisa
  ter Node/Postgres já no servidor, tudo isolado em containers.
...
### 2.1. Baixar o código
```
git clone https://github.com/yLucasG/RepositorioPMPE.git
cd RepositorioPMPE
```
...
```
Isso builda a imagem (primeira vez demora alguns minutos) e sobe dois containers:
o banco Postgres e a aplicação. Na primeira subida, o banco já nasce com as tabelas
criadas automaticamente (não precisa rodar nenhum comando de migração à parte).
...
docker compose up -d --build
```
O banco não é afetado (o volume persiste); só o código da aplicação é reconstruído.
...
`DATABASE_URL` comum e os PDFs são salvos numa pasta comum do disco.
...
### 7.4. Instalar dependências e criar as tabelas
```
npm ci
```
...
```
Isso gera o Angular buildado em `dist/repositorio-angular/browser` e a API
compilada em `dist-server/`.

### 7.6. Testar manualmente antes de deixar permanente
```
node dist-server/server.js
```
Deve aparecer `Servidor rodando em http://localhost:3000`. Acesse
`http://localhost:3000` (ou o IP do servidor) pra conferir. Ctrl+C encerra — isso é
só o teste, o passo seguinte deixa isso rodando de verdade em segundo plano.

### 7.7. Deixar o processo no ar permanentemente
...
[Service]
Type=simple
WorkingDirectory=/caminho/completo/para/repositorio-angular
...
```
O `.env` da seção 7.3 é lido automaticamente (a aplicação usa `dotenv`), desde que
...
nssm set RepositorioDTEC AppDirectory "C:\caminho\completo\para\repositorio-angular"
...
### 7.8. Criar o primeiro usuário admin
Não existe tela de cadastro (proposital). Na pasta do projeto, rode:
```
...
npx tsc -p tsconfig.server.json
```
Depois reinicie o serviço (`sudo systemctl restart repositorio-dtec` no Linux, ou
`nssm restart RepositorioDTEC` no Windows). O banco não é afetado — só o código da
aplicação é reconstruído.
